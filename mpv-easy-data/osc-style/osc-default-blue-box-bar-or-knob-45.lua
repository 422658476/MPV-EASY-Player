...
--修改处
    windowcontrols_title = "MPV-EASY Player - ${filename}", -- same as title but for windowcontrols
    greenandgrumpy = false,     -- disable santa hat
    livemarkers = true,         -- update seekbar chapter markers on duration change
    chapter_fmt = "Chapter: %s", -- chapter print format for seekbar-hover. "no" to disable
    unicodeminus = false,       -- whether to use the Unicode minus sign character
--修改处
    background_color = "#393939",     -- background color of the osc
--修改处
    timecode_color = "#009ADD",       -- color of the progress bar and time color
...
local function set_osc_styles()
    osc_styles = {
--修改处
        bigButtons = "{\\blur0\\bord0\\1c&H" .. osc_color_convert(user_opts.buttons_color) .. "\\3c&HFFFFFF\\fs38\\fnmpv-osd-symbols}",
...
--修改处
        smallButtonsR = "{\\blur0\\bord0\\1c&H" .. osc_color_convert(user_opts.small_buttonsR_color) .. "\\3c&HFFFFFF\\fs24\\fnmpv-osd-symbols}",
...
--修改处
        vidtitle = "{\\blur0\\bord0\\1c&H" .. osc_color_convert(user_opts.title_color) .. "\\3c&HFFFFFF\\fs17\\q2}",
--修改处
        box = "{\\rDefault\\blur0\\bord0\\1c&H" .. osc_color_convert(user_opts.background_color) .. "\\3c&H000000}",
...
--修改处
        timePosBar = "{\\blur0\\bord".. user_opts.tooltipborder .."\\1c&H" .. osc_color_convert(user_opts.time_pos_color) .. "\\3c&H" .. osc_color_convert(user_opts.time_pos_outline_color) .. "\\fs24}",
...
--修改处
        wcTitle = "{\\1c&H" .. osc_color_convert(user_opts.title_color) .. "\\fs20\\q2}",
...
--修改处
	    buttontext = buttontext.gsub(buttontext,"%([^'':()]+[^'':()]+[^'':()]+%)[ ]", "")
	    buttontext = buttontext.gsub(buttontext,"%[[^'':()m]+[^'':()p]+[^'':()v]+%][ ]", "")
...
--修改处
    lo.geometry =
        { x = titlebox_left + left_pad, y = wc_geo.y - 3 -2, an = 1,
...
--box左上角y坐标 posY - pos_offsetY 但an=5 元素中心点坐标和这个坐标一致，所以还要根据an=？加减元素的高度
    ...
    -- Seekbar
    --
	geo ={x = posX, y = posY - osc_h / 2 +14, an = 5, w = osc_w - 16 , h = 14}
    ...
    lo.geometry = geo
    lo.layer = 15
    lo.style = osc_styles.timePosBar
    lo.alpha[1] = 60
	lo.box.radius = 3
    ...
    lo = add_layout("playlist_prev")
    lo.geometry = {x = rjno1X + 10 * 3.5 * 0 + 10 * 3 -8, y = rjno1Y + 10 * 1.6 + 50 / 2 + 4+4, an = 5, w = 30, h = 50}
    lo.style = osc_styles.smallButtonsL
	
    lo = add_layout("chapter_prev")
    lo.geometry = {x = rjno1X + 10 * 3.5 * 1 + 10 * 3 -8, y = rjno1Y + 10 * 1.6 + 50 / 2  + 4+4, an = 5, w = 30, h = 50}
    lo.style = osc_styles.smallButtonsBar
	
	lo = add_layout("skip_backward")
    lo.geometry = {x = rjno1X + 10 * 3.5 * 2 + 10 * 3 -8, y = rjno1Y + 10 * 1.6 + 50 / 2  + 4+4, an = 5, w = 30, h = 50}
    lo.style = osc_styles.smallButtonsBar

	
    lo = add_layout("play_pause")
    lo.geometry = {x = rjno1X + 10 * 3.5 * 3 + 10 * 3 -8, y = rjno1Y + 10 * 1.6 + 50 / 2  + 4+4, an = 5, w = 30, h = 50}
    lo.style = osc_styles.bigButtons	
	
		
    lo = add_layout("skip_forward")
    lo.geometry = {x = rjno1X + 10 * 3.5 * 4 + 10 * 3 -8, y = rjno1Y + 10 * 1.6 + 50 / 2  + 4+4, an = 5, w = 30, h = 50}
    lo.style = osc_styles.smallButtonsBar

	
	lo = add_layout("chapter_next")
    lo.geometry = {x = rjno1X + 10 * 3.5 * 5 + 10 * 3 -8, y = rjno1Y + 10 * 1.6 + 50 / 2  + 4+4, an = 5, w = 30, h = 50}
    lo.style = osc_styles.smallButtonsBar	
	
	

    lo = add_layout("playlist_next")
    lo.geometry = {x = rjno1X + 10 * 3.5 * 6 + 10 * 3 -8, y = rjno1Y + 10 * 1.6 + 50 / 2  + 4+4, an = 5, w = 30, h = 50}
    lo.style = osc_styles.smallButtonsL	
	
	
--an = 5 时文字居中显示 往2边增加，所以一定要用1，这样多出的文字只会往右侧增长
	lo = add_layout("title")
    lo.geometry = {x = rjno1X + 10 * 26 , y = rjno1Y + 10 * 2.6 + 20 / 2  + 3+4, an = 1, w = 1100 - 10 * 3.5 * 7, h = 20}
    lo.style = osc_styles.vidtitle	
    --lo.button.maxchars = user_opts.boxmaxchars
...
--an = 1 这样左侧的时间变长才会往右，但y轴要调整
    lo = add_layout("tc_left")
    lo.geometry = {x = rjno1X + 10 * 26, y = rjno1Y + 10 * 5.1+ 20 / 2 - 1 + 4+4, an = 1, w = 110, h = 20}
    lo.style = osc_styles.vidtitle	

--an = 5
    lo = add_layout("cache")
    lo.geometry = {x = rjno1X + 10 * 50 + 10 * 11 / 2, y = rjno1Y + 10 * 5.1 + 1 + 4+4, an = 5, w = 110, h = 20}
    lo.style = osc_styles.vidtitle	
--an = 9 这样右侧的时间变长才会往左，但y轴要调整
    lo = add_layout("tc_right")
    lo.geometry = {x = rjno1X + 10 * 78 + 10 * 11 / 2, y = rjno1Y + 10 * 5.1- 20 / 2 + 4 + 4+4, an = 9, w = 110, h = 20}
    lo.style = osc_styles.vidtitle
    ...
    lo = add_layout("audio_track")
    lo.geometry = {x = rjno1X + 10 * 85 + 10 * 7/2, y = rjno1Y + 10 * 5.1 + 4+4, an = 5, w = 70, h = 20}
    lo.style = osc_styles.smallButtonsR

    lo = add_layout("sub_track")
    lo.geometry = {x = rjno1X + 10 * 93 + 10 * 7/2, y = rjno1Y + 10 * 5.1 + 4+4, an = 5, w = 70, h = 20}
    lo.style = osc_styles.smallButtonsR

--an = 1 图标最左侧才能点击
    lo = add_layout("volume")
    lo.geometry = {x = rjno1X + 10 * 102 , y = rjno1Y + 10 * 5 + 13 + 4+4, an = 1, w = 30, h = 26}
    lo.style = osc_styles.smallButtonsR
--an = 1 图标最左侧才能点击
    lo = add_layout("tog_fs")
    lo.geometry = {x = rjno1X + 10 * 106, y = rjno1Y + 10 * 5 + 14 + 4+4 , an = 1, w = 30, h =26 }
    lo.style = osc_styles.smallButtonsR	
